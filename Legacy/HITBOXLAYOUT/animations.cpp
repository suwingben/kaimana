  ...
  while(counter >=0)
  {
    ...
    if( ((counter+(FIREBALL_OFFSET_1)) < FIREBALL_SIZE) && ((counter+(FIREBALL_OFFSET_1)) >= 0) )
    {
      ...
      kaimana.setLED(
        ...
        // no red just cyan used for fireball
        pgm_read_byte_near(&colorCycleData[(counter+(FIREBALL_OFFSET_1))%FIREBALL_SIZE]),
        pgm_read_byte_near(&colorCycleData[(counter+(FIREBALL_OFFSET_1))%FIREBALL_SIZE])
      );
    }
...
// Tatsumaki Senpukyaku (Hurricane Kick)(RASENHA)
//
...
// Ultra 1 (Metsu Hadouken)(MEIOKEN)
//
...
// Ultra 2 (Metsu Shoryuken)(V SKILL)
//
