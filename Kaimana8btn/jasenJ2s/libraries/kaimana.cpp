...
void Kaimana::setLED(int index,int alpha, int iR, int iG, int iB)
{
  // set led identified by index to the RGB color passed to this function
 
  _led[index].r = iR*alpha;
  _led[index].g = iG*alpha;
  _led[index].b = iB*alpha;
  ...
  else
  {
     ...
     _led[index].g = iG*alpha;
     _led[index].b = iB*alpha;
     _led[index+ 1].r = iR*alpha;
     _led[index+ 1].g = iG*alpha;
     _led[index+ 1].b = iB*alpha;
   }
